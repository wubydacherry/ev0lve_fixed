...
namespace evo::gui
{
	...
	{
	public:
		control_container(control_id id) : control(std::move(id))
		{
			is_container = true;
			margin = ren::rect();
		...
		control_container(control_id _id, const ren::vec2 &_pos, const ren::vec2 &_size) : control(std::move(_id), _pos, _size)
		{
			is_container = true;
			margin = ren::rect();
			type = ctrl_control_container;
		...
		ren::vec2 first_offset{}; // first item offset
		ren::vec2 last_offset{}; // last item offset
