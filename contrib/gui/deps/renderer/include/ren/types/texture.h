...
namespace evo::ren
{
	...
	public:
		...
		explicit texture(const char *path) : src(file), path(path) { REN_DBG_FMT("texture allocated (path %s)", path); }
		texture(void *data, uint32_t sz) : src(mem), data(data), size(sz) { REN_DBG_FMT("texture allocated (auto, mem %p : %ul)", data, sz); }
		texture(void *data, uint32_t w, uint32_t h, uint32_t p) : src(rgba), data(data), width(w), height(h), pitch(p)
		{
			REN_DBG_FMT("texture allocated (rgba, mem %p, w %ul, h %ul, p %ul)", data, w, h, p);
		}

		~texture() { REN_DBG("texture freed"); }
	...
	public:
		explicit svg_texture(const std::string &sv, float h = 0.f) : svg(sv), texture(nullptr, 0)
		{
			...
			REN_DBG_FMT("svg texture allocated (target height %u, real %d %d)", height, real_w, real_h);
		}

		~svg_texture() { REN_DBG("svg texture freed"); }
