...
std::shared_ptr<control> container::find(uint64_t id)
{
	...
	for_each_control(
		[&result, id](std::shared_ptr<control> &c)
		{
			if (result || !c)
				return;
...
void container::process_remove_schedule()
{
	...
	{
		if (auto p = std::find_if(controls.begin(), controls.end(),
								  [c](const std::shared_ptr<control> &ctrl) { return ctrl->id == c; });
			p != controls.end())
		{
...
{
	std::sort(controls_ordered.begin(), controls_ordered.end(),
			  [](const std::shared_ptr<control> &a, const std::shared_ptr<control> &b)
			  {
