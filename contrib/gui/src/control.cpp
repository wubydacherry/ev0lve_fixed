...
{
	return ctx->show_ids.get() && !id_string.empty() && type != ctrl_layout && type != ctrl_window &&
		   type != ctrl_window && type != ctrl_widget && type != ctrl_tab && type != ctrl_tabs_layout &&
		   type != ctrl_tabs_layout && type != ctrl_selectable && !disable_id_display;
}
...
void control::render()
{
	...
	if (is_mouse_on_me && !is_locked_by_someone_else())
	{
		...
		if (can_show_debug_info() && ctx->get_last_hovered() == id)
		{
			...
			d->g.texture = {};
			d->font = m;
			d->add_text(vec2(r.tl().x + 28.f, r.tl().y + 3.f), id_string, colors.text);
			d->font = m_normal;
			d->add_text(vec2(r.tl().x + 28.f, r.tl().y + 19.f), XOR("Middle click to copy"), colors.text_dark);
			d->add_rect(r, colors.outline);
		}

		// show tooltip
		if (!tooltip.empty() && !ctx->show_ids.get())
		{
	...
	if (highlight_alpha->value > 0.f)
	{
		...
		const auto &d = draw.layers[ctx->scrollbar_layer];
		d->add_rect(r, colors.accent.mod_a(highlight_alpha->value), 2.f);
	}
...
bool control::is_locked_by_me() { return is_input_locked() && ctx->active->id == id; }

bool control::is_locked_by_someone_else() { return is_input_locked() && ctx->active->id != id; }
...
{
	return p.x < -(size.x - max_offset) || p.y < -(size.y - max_offset) || p.x > draw.display.x - max_offset ||
		   p.y > draw.display.y - max_offset;
}
...
rect control::area(bool m) const { return rect(pos + margin.mins).size(size + (m ? margin.maxs : vec2())); }

rect control::area_abs(bool m) const { return rect(pos_abs()).size(size + (m ? margin.maxs : vec2())); }
