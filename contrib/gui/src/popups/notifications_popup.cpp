...
void notifications_popup::on_first_render_call()
{
	...
	const auto arr = std::make_shared<layout>(control_id{GUI_HASH("gui_nc_list"), ""}, vec2{1.f, 60.f},
											  vec2{size.x - 2.f, size.y - 61.f}, s_vertical);

	auto odd = false;
	notify.for_each(
		[&arr, &odd](const std::shared_ptr<notification> &n) {
			arr->add(
				std::make_shared<notification_control>(control_id{GUI_HASH("gui_nc_n") + n->id, ""}, odd = !odd, n));
		});
	...
	const auto clear =
		std::make_shared<button>(control_id{GUI_HASH("gui_nc_clear"), ""}, XOR("Clear"), ctx->res.icons.notify_clear,
								 vec2{size.x - 80.f, 16.f}, vec2{40.f, 20.f});
...
void notifications_popup::render()
{
	...
	if (!notify.get_count())
		d->add_text(r.center() + vec2{0.f, 20.f}, XOR("No new notifications"), colors.text_dark,
