...
namespace evo::ren
{
	void add_with_blur(std::shared_ptr<layer> &d, const rect &area, const std::function<void(std::shared_ptr<layer> &)> &fn)
	{
		...
		const auto ds_display = draw.display * .25f;
...
context::context(float text_size)
{
	...
	draw.shaders[GUI_HASH("blur_f")] = std::make_shared<shader>(
		XOR("sampler s0;const float2 resolution:register(c0);const float time:register(c1);const float "
			"alpha:register(c2);float4 "
			"main(float2 "
			"uv:TEXCOORD0):COLOR0{float radius=2.0;float2 "
			"invSize=1.0/resolution.xy;float weight=0.0;float4 color=0.0;for(float "
			"x=-radius;x<=radius;x+=1.0){for(float y=-radius;y<=radius;y+=1.0){float2 "
			"coord=uv+float2(x,y)*invSize;color+=tex2D(s0,coord)*exp(-((x*x+y*y)/"
			"(2.0*radius*radius)));weight+=exp(-((x*x+y*y)/(2.0*radius*radius)));}}color.a*=alpha;return "
			"color/weight;}"),
		shader_frag);
...
bool context::refresh(UINT msg, WPARAM w, LPARAM l)
{
	// take input
	...
	if (input.did_process_key() && should_process_hotkeys && !is_active_hotkey)
	{
		process_hotkeys();
		process_chords();
	}
...
void context::render()
{
	...
	{
		if (colors.accents.accent != accent_prev)
		{
			colors.accents.accent_dark = colors.accents.accent.darken(.32f);
			colors.accents.accent_darker = colors.accents.accent.darken(.39f);

			for (auto &i : accents)
			{
				const auto c = i.lock();
				if (c && c->end == accent_prev)
					c->direct(colors.accents.accent);
			}
			...
			for (auto &i : accents_float)
			{
				...
				if (c && c->end.c == accent_prev)
					c->direct(u_fc(4.f, colors.accents.accent));
			}

			accent_prev = colors.accents.accent;
		}
...
{
	// reset all controls
	for_each_control([](std::shared_ptr<control> &c) { c->reset(); });
}

void context::track_accent_anim(const std::weak_ptr<ren::anim_color> &a)
{
	std::lock_guard _(accents_mutex);
...
{
	if (auto p = std::find_if(controls.begin(), controls.end(), [id](const std::shared_ptr<control> &c) -> bool { return c->id == id; }); p != controls.end())
	{
		const auto &c = *p;
		if (c->is_container)
			c->as<control_container>()->remove_all();
...
void context::add_chord(const chord &ch)
{
	...
	if (const auto i = std::ranges::find_if(
			chords,
			[&](const chord_info &c)
			{ return c.ch.mods == ch.mods && std::ranges::count_if(c.ch.keys, [&](char k) { return std::ranges::find(ch.keys, k) != ch.keys.end(); }); });
		i != chords.end())
		throw; // nope
...
void context_debug::render(const std::shared_ptr<control> &c)
{
	...
	if (c->is_mouse_on_me)
	{
		...
		char buffer[128]{};
		sprintf_s(
			buffer, 128, XOR("%.0f:%.0f <%.0f:%.0f> : <%.0f:%.0f> / %0*X / %i"), c->pos_abs().x, c->pos_abs().y, c->pos.x, c->pos.y, c->size.x, c->size.y, 8, c->id,
			c->sort);
		...
		d->add_rect_filled(area, fill);
		d->font = fnt;
		d->add_text(area.tl() + 2.f, buffer, color::white());
	}

	d->add_rect(c->area_abs(), fill.mod_a(c->is_mouse_on_me ? 255 : 25));
}
